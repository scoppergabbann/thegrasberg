  ...
  return (
    <div className='max-w-3xl mx-auto space-y-4 sm:space-y-5'>
      <div>
        ...
        <p className='text-xs sm:text-sm text-zinc-500 mt-0.5'>Review perjalanan trading — analisa, eksekusi, dan pelajaran</p>
      </div>
      ...
      {notes.length === 0 ? (
        ...
        <ul className='space-y-3' role='list'>
          {notes.map(note => {
            ...
            return (
              <li key={note.note_date} className='card p-3 sm:p-4 space-y-3 hover:border-zinc-700 transition-colors'>
                <div className='flex items-start justify-between gap-3'>
                  ...
                  <span className={cn('mono text-xs sm:text-sm font-semibold shrink-0', pnlColor(pnl))}>{formatPnl(pnl)}</span>
                </div>
