...
export function computePsychResult(scores: number[]): Omit<PsychResult,'id'|'created_at'|'result_date'> {
  const score    = scores.reduce((a,b) => a+b, 0)
  const maxScore = PSYCH_QUESTIONS.length * 3
  const pct      = Math.round(score / maxScore * 100)
  let verdict: PsychVerdict, allowed: boolean, feedback: PsychResult['feedback']
  if (score >= 20) {
    verdict='SIAP TRADING'; allowed=true
    feedback=[{type:'good',text:'Kondisi mental dan fisik sangat optimal.'},{type:'good',text:'Persiapan analisa matang, risk management terdefinisi.'}]
  } else if (score >= 14) {
    verdict='KONDISI CUKUP'; allowed=true
    feedback=[{type:'warn',text:'Kondisi cukup baik, ada beberapa area perlu diperhatikan.'},{type:'warn',text:'Pertimbangkan reduce lot size, hindari news event.'}]
  } else if (score >= 7) {
    verdict='WASPADA'; allowed=false
    feedback=[{type:'bad',text:'Kondisi mental kurang optimal.'},{type:'bad',text:'Disarankan hanya observe, jangan buka posisi.'},{type:'warn',text:'Jika memaksa, lot sangat kecil dan strict SL.'}]
  } else {
    verdict='JANGAN TRADING'; allowed=false
    feedback=[{type:'bad',text:'Kondisi sangat tidak ideal untuk trading.'},{type:'bad',text:'Risiko kerugian psikologis sangat tinggi.'},{type:'bad',text:'Gunakan waktu untuk review chart atau istirahat.'}]
  }
